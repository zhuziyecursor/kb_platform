...
本项目对开发服务器端口有严格要求，**禁止**使用以下常见端口：
- 前端：`3000`、`4200`、`5173`、`8080`、`5000`
- 后端：`8000`、`8080`、`8085`、`9000`、`5000`

## 前端端口要求
- 必须是 **4 位数字**（范围 1000–9999）
- 且不等于上述禁止列表中的端口
- 推荐从候选池中选取一个未被占用的端口，例如 `1720`、`3402`、`5721`、`8945`

## 后端端口要求
- 必须是 **30000 及以上的数字**（范围 30000–65535）
- 且不得使用 `30000` 本身？如果不希望使用 30000，可以写明“30000 以下不包括”或“大于 30000”。下面明确为 **大于等于 30001**
- 推荐候选：`31000`、`32500`、`34000`、`50001`、`62222`

## AI 行为指引
1. 在生成项目配置文件（如 `vite.config.js`、`webpack.config.js`、`package.json` 的 `dev` 脚本、`.env` 等）时，**必须**将端口设为符合上述规则的值。
2. 如果使用脚手架工具（create-react-app、vue-cli 等），应在启动命令中显式指定端口，例如 `--port 3402`。
3. 不要在代码注释或文档中推荐使用默认端口。
4. 启动服务后，应在控制台输出实际使用的端口号，以方便开发者查看。
5. 中间件的端口可以遵从这个规范，因为再部署的时候就已经分配好了